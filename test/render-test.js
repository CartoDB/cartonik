...
describe('Render ', function () {
  it('getTile() "jpeg:quality=20" format', async function () {
    ...
    await assert.imageEqualsFile(tile, './test/fixtures/output/pngs/world-jpeg20.jpeg', 0.05, 'jpeg:quality=20')
  ...
  describe('getTile() ', function () {
    ...
    before(function () {
      renderer = rasterRendererFactory({ xml: fs.readFileSync('./test/fixtures/mmls/world.xml', 'utf8'), base: './test/fixtures/datasources/shapefiles/world-borders/' })
    })
    ...
    it('validates', async function () {
      ...
      for (const coords of tileCoords) {
        ...
        await assert.imageEqualsFile(tile, './test/fixtures/output/pngs/transparent_' + key + '.png')
  ...
  describe('getTile() with buffer-size', function () {
    ...
    before(function () {
      renderer = rasterRendererFactory({
        xml: fs.readFileSync('./test/fixtures/mmls/world_labels.xml', 'utf8'),
    ...
    it('validates buffer-size', async function () {
      ...
      for (const coords of tiles) {
        const { tile, headers } = await renderer.getTile('png', coords[0], coords[1], coords[2])
        const key = coords[0] + '_' + coords[1] + '_' + coords[2]
        const filepath = './test/fixtures/output/pngs/buffer_size_' + key + '.png'
  ...
  describe('Works with render time variables', function () {
    TESTCOLOR.forEach(function (customColor) {
      it('Color ' + customColor, async function () {
        const options = {
          xml: fs.readFileSync('./test/fixtures/mmls/world_variable.xml', 'utf8'),
          base: './test/fixtures/datasources/shapefiles/world-borders/',
        ...
        await assert.imageEqualsFile(tile, './test/fixtures/output/pngs/transparent_2_2_2_' + customColor + '.png')
        assert.strictEqual(headers['Content-Type'], 'image/png')
        await renderer.close()
      })
    })

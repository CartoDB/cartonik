...
module.exports = function rasterRendererFactory (uri, callback) {
  const renderer = new RasterRenderer(uri, callback)
  return renderer
}

module.exports.mapnik = mapnik

function RasterRenderer (uri, callback) {
  this.open = false
...
RasterRenderer.registerProtocols = function (tilelive) {
  tilelive.protocols['mapnik:'] = RasterRenderer
}

RasterRenderer.prototype._open = function (callback) {
...
RasterRenderer.prototype.close = function (callback) {
  if (!this.open) {
    return callback()
  }
...
RasterRenderer.prototype.getTile = function (z, x, y, callback) {
  const format = this._format || 'png'
  this._renderTile(format, z, x, y, callback)
}

RasterRenderer.prototype.getGrid = function (z, x, y, callback) {
  const format = 'utf'
  this._renderTile(format, z, x, y, callback)
}

RasterRenderer.prototype._renderTile = function (format, z, x, y, callback) {
  z = +z
  x = +x
  y = +y
...
RasterRenderer.prototype._renderMetatile = function (format, z, x, y, metatile, callback) {
  this._mapPool.acquire()
    .then((resource) => {
      if (!(resource instanceof mapnik.Map)) {
        const err = resource
...
RasterRenderer.prototype._sliceMetatile = function (image, options, metatile, stats, callback) {

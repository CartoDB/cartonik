...
class RasterRenderer {
  ...
  async renderMetatile (format, z, x, y, metatile) {
    ...
    try {
      ...
      const tiles = await sliceMetatile(map, tile, options, metatile, stats)
...
async function sliceMetatile (map, image, options, metatile, stats) {
  const tiles = await Promise.all(metatile.tiles.map(coords => slice(coords, map, image, options, metatile, stats)))
  return Object.assign({}, ...tiles)
}

async function slice (coords, map, image, options, metatile, stats) {

...
module.exports = function vectorRendererFactory (uri, callback) {
  const renderer = new VectorRenderer(uri, callback)
  return renderer
}

function VectorRenderer (uri, callback) {
  if (!uri.xml) {
...
VectorRenderer.registerProtocols = function (tilelive) {
  tilelive.protocols['VectorRenderer:'] = VectorRenderer
}

VectorRenderer.prototype.close = function (callback) {
  this._mapPool.drain()
...
VectorRenderer.prototype.getTile = function (z, x, y, callback) {
